...
printf("144 / (x * y) = %d \n", 144 / (x * y));
	...
	void Task5()
	{
		...
		if (x < 0 || y < 0 || z < 0) // проверка значений x,y,z > 0
		{printf("Вы ввели одно из значений отрицательным. Попробуйте еще раз...\n\n"); goto start;}

					if (x < y && x < z && x != y && x != z) // определяем меньшее значение из x,y,z и присваиваем его k
					{
						printf("\nx=%d меньшее из x,y,z тогда k=%d\n", x, x);
						k = (double)x;
					}
					else { printf("\nНевозможно определить одно меньшее значение,\nт.к. введены два минимальных значения.\nПопробуйте еще раз...\n\n"); goto start; }
				
					if (y < x && y < z && y != x && y != z) // определяем меньшее значение из x,y,z и присваиваем его k
					{
						printf("\nx=%d меньшее из x,y,z тогда k=%d\n", y, y);
						k = (double)y;
					}
